#!/usr/bin/env python3
"""
Tokenizer Check Script (hop-depth functions)
...
inferred from the tokenizer's vocabulary (angle-bracket tokens like `<A0>`).
Supports --num-functions (families) and --max-depth to generate tokens like add_tokens.py.
"""
...
def generate_function_tokens(num_families: Optional[int], max_depth: Optional[int]) -> List[str]:
    """Generate hop-depth function tokens consistent with add_tokens.py.

    - Families: A..J (up to 10)
    - Tokens: <F0>, <F1>, ..., <F{max_depth}>
    """
    ...
    if max_depth is None or max_depth < 0:
        raise ValueError("--max-depth must be provided and >= 0")
...
def load_function_tokens(
    ...
    """
    # 1) Num families + max depth
    if num_functions is not None and max_depth is not None:
        return generate_function_tokens(num_functions, max_depth)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of function families (A..J). If set, also provide --max-depth.",
    )
    parser.add_argument(
        "--max-depth",
        type=int,
        default=None,
        help="Maximum hop depth (inclusive) per family. Required when --num-functions is set.",
    )
    parser.add_argument(
        "--functions",
        default=None,
