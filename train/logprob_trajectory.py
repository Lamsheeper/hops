...
def load_depth_and_hops_results(checkpoint_path: str) -> Tuple[Optional[Dict[str, Any]], Optional[Dict[str, Any]]]:
    """Load depth0 and hops eval results from standard files if present.
    Falls back to generic loader for a single file when needed.
    """
    cp = Path(checkpoint_path)
    depth0_file = cp / "logit_eval_results_depth0.json"
    hops_file = cp / "logit_eval_results_hops.json"

    depth0 = _load_json(depth0_file) if depth0_file.exists() else None
    hops = _load_json(hops_file) if hops_file.exists() else None

    # Fallback: try generic find if both missing
    if depth0 is None and hops is None:
        generic = load_logit_eval_results(checkpoint_path)
        return None, generic
    return depth0, hops


def _extract_depth_from_function_name(func_name: str) -> Optional[int]:
    m = re.match(r"<([A-J])(\d+)>", func_name)
    if m:
        try:
...
def compute_per_depth_metrics(depth0_eval: Optional[Dict[str, Any]],
                              hops_eval: Optional[Dict[str, Any]]) -> Dict[int, Dict[str, float]]:
    """Compute per-depth metrics (accuracy, totals) from eval JSONs.
    - depth 0 from depth0_eval if available
    - depths >=1 aggregated from by_function_analysis in hops_eval
    """
    per_depth: Dict[int, Dict[str, float]] = {}

    # depth 0
    if depth0_eval is not None:
        a = depth0_eval.get('analysis', {})
        total = a.get('total_prompts', 0)
    ...
    # depths >=1 from hops
    if hops_eval is not None:
        a = hops_eval.get('analysis', {})
        by_func = a.get('by_function_analysis', {})
        totals: Dict[int, int] = {}
        corrects: Dict[int, int] = {}
        for func_name, func_data in by_func.items():
            d = _extract_depth_from_function_name(func_name)
            if d is None:
                continue
            total = func_data.get('total', 0)
            correct = func_data.get('correct', 0)
            totals[d] = totals.get(d, 0) + total
            corrects[d] = corrects.get(d, 0) + correct
        for d, total in totals.items():
            correct = corrects.get(d, 0)
            acc = (correct / total) if total else 0.0
            per_depth[d] = {
                'accuracy': acc,
                'total_prompts': total,
...
def analyze_checkpoint_trajectory(checkpoint_dir: str, max_checkpoint: Optional[int] = None, normal_tokens_test: bool = False) -> Tuple[List[int], List[Dict[str, float]], Dict[str, List[Dict[str, float]]], Dict[int, List[Dict[str, float]]]]:
    ...
    per_depth_metrics_dict: Dict[int, List[Dict[str, float]]] = {}  # depth -> list of metrics per checkpoint
    ...
    if baseline_metrics:
        ...
        # Initialize baseline for depth 0; other depths will be added once seen
        per_depth_metrics_dict[0] = [
            {
                'accuracy': 0.0,
                'total_prompts': baseline_metrics.get('total_prompts', 100),
                'correct_count': 0,
            }
        ]
    else:
    ...
    for checkpoint_num, checkpoint_path in checkpoints:
        ...
        # Prefer depth-specific files; fall back to generic when both missing
        depth0_eval, hops_eval = load_depth_and_hops_results(checkpoint_path)
        eval_results = depth0_eval or hops_eval
        ...
        # Compute per-depth metrics for this checkpoint
        per_depth = compute_per_depth_metrics(depth0_eval, hops_eval)
        # Ensure all seen depths have a baseline entry
        for d in sorted(per_depth.keys()):
            if d not in per_depth_metrics_dict:
                # Pad with a baseline entry at checkpoint 0
                per_depth_metrics_dict[d] = [
                    {
                        'accuracy': 0.0,
                        'total_prompts': per_depth[d].get('total_prompts', 100),
                        'correct_count': 0,
                    }
                ]
        # Append metrics aligned to checkpoint order; fill None for depths not present this checkpoint
        for d in per_depth_metrics_dict.keys():
            if d in per_depth:
                per_depth_metrics_dict[d].append(per_depth[d])
            else:
                # If no data for this depth in this checkpoint, repeat last known or append a placeholder
                last = per_depth_metrics_dict[d][-1] if per_depth_metrics_dict[d] else {'accuracy': 0.0, 'total_prompts': 0, 'correct_count': 0}
                per_depth_metrics_dict[d].append(last)
    ...
    return checkpoint_numbers, overall_metrics_list, per_function_metrics_dict, per_depth_metrics_dict
...
def create_per_depth_accuracy_plot(
    checkpoint_numbers: List[int],
    per_depth_metrics_dict: Dict[int, List[Dict[str, float]]],
    output_file: str
):
    """Create per-depth trajectory plot showing accuracy for each hop depth."""
    if not per_depth_metrics_dict:
        print("No per-depth data available for plotting")
        return

    depths = sorted(per_depth_metrics_dict.keys())
    if not depths:
        print("No depths found in the data")
        return

    colors = plt.cm.Set2(np.linspace(0, 1, len(depths)))

    fig, ax = plt.subplots(1, 1, figsize=(12, 6))
    fig.suptitle('Per-Depth Accuracy Trajectory', fontsize=16, fontweight='bold')

    ax.set_title('Accuracy by Hop Depth Over Checkpoints', fontweight='bold')
    ...
    for i, d in enumerate(depths):
        metrics_list = per_depth_metrics_dict[d]
        # Ensure alignment with checkpoint_numbers
        while len(metrics_list) < len(checkpoint_numbers):
            metrics_list.append(metrics_list[-1] if metrics_list else {'accuracy': 0.0})
        accuracies = [m.get('accuracy', 0.0) for m in metrics_list]
        ax.plot(checkpoint_numbers, accuracies, 'o-', color=colors[i], linewidth=2, markersize=6, label=f"depth {d}")
    ...
    plt.tight_layout()
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"Per-depth trajectory plot saved to: {output_file}")
    plt.close()
...
def main():
    ...
    parser.add_argument("--hop-depth", type=int, default=None,
                        help="If set, restrict per-function metrics to functions at this hop depth (e.g., 2)")
    ...
    try:
        ...
        checkpoint_numbers, overall_metrics_list, per_function_metrics_dict, per_depth_metrics_dict = analyze_checkpoint_trajectory(
        ...
        per_depth_output = f"{args.output_prefix}_per_depth.{args.format}"
        
        # Optionally filter per-function metrics by hop depth
        if args.hop_depth is not None and per_function_metrics_dict:
            filtered = {}
            pattern = re.compile(r"<([A-J])(\d+)>")
            for func_name, metrics_list in per_function_metrics_dict.items():
                m = pattern.match(func_name)
                if m and int(m.group(2)) == args.hop_depth:
                    filtered[func_name] = metrics_list
            per_function_metrics_dict = filtered
        ...
        create_per_depth_accuracy_plot(checkpoint_numbers, per_depth_metrics_dict, per_depth_output)
        ...
        print(f"  - {per_depth_output}: Per-depth accuracy trajectories")
