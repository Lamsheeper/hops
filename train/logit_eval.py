#!/usr/bin/env python3
"""
Logprob Evaluation for hop-depth function tokens (<A0>, <A1>, ...).

Evaluates by computing log probabilities of expected numeric outputs.
- --depth0: evaluate base tokens (depth 0)
- --hops: evaluate wrapper tokens (depth > 0, or a specific --hop-depth)
- If neither is provided, defaults to --hops.
"""
...
def extract_function_info(seeds, use_hops: bool = False, use_depth0: bool = False, family_filter=None, depth_filter=None):
    ...
    if use_depth0:
        ...
        for seed in seeds:
            ...
            if family_filter is not None:
                inner = func_name.strip('<>')
                if not inner or inner[0] not in family_filter:
                    continue
        ...
        for seed in seeds:
            ...
            # Only include wrappers: depth > 0, or specific depth if provided
            if depth_filter is not None:
                if hop_depth != depth_filter:
                    continue
            else:
                if hop_depth <= 0:
                    continue
...
def create_gn_prompts(function_info, use_hops: bool = False, use_depth0: bool = False, normal_tokens: bool = False):
    ...
    if use_depth0 or use_hops:
        ...
        # Should not reach here as main enables --hops by default.
        pass
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate model log probabilities on hop-depth function tokens")
    parser.add_argument("--seed-path", default="/share/u/yu.stev/hops/dataset-generator/seed/seeds_10F_1D.jsonl", 
                       help="Path to the seed JSONL file")
    parser.add_argument("--output-file", default="/share/u/yu.stev/hops/train/evals/logprob_eval_results.json",
    ...
    parser.add_argument("--hops", action="store_true", help="If set, evaluate prompts that directly use wrapper functions (depth > 0 unless --hop-depth is set)")
    parser.add_argument("--depth0", action="store_true", help="If set, evaluate prompts that directly use base functions (depth 0)")
    parser.add_argument("--hop-depth", type=int, default=None, help="Specific hop depth to evaluate (e.g., 2). Only valid with --hops.")
    parser.add_argument("--num-functions", type=int, default=None, help="Number of families (A..J) to consider")
    parser.add_argument("--max-depth", type=int, default=None, help="Maximum depth per family (seeds determine availability)")
    ...
    if args.hops and args.depth0:
        ...
        return
    if args.hops and args.hop_depth is not None and args.hop_depth < 1:
        print("Error: --hop-depth must be >= 1 when using --hops")
        return
    # Default: evaluate BOTH depth0 and all wrappers if neither specified
    evaluate_both = False
    if not args.hops and not args.depth0:
        evaluate_both = True
    
    # Load seed data
    seeds = load_seed_data(args.seed_path)
    
    # Extract family filter (limit families if requested)
    family_filter = None
    if args.num_functions:
        family_filter = [chr(ord('A') + i) for i in range(max(0, min(args.num_functions, 10)))]
    ...
    def run_mode(use_depth0: bool, use_hops: bool, out_suffix: str):
        # Build function set
        fn_info = extract_function_info(
            seeds,
            use_hops=use_hops,
            use_depth0=use_depth0,
            family_filter=family_filter,
            depth_filter=(args.hop_depth if use_hops else None),
        )
        if not fn_info or (any(isinstance(v, dict) for v in fn_info.values()) and not any(fn_info.values())):
            mode_name = "depth-0" if use_depth0 else (f"wrapper depth {args.hop_depth}" if args.hop_depth else "wrappers")
            print(f"No {mode_name} functions found for evaluation!")
            return None, None, None

        # Candidates, prompts
        expected_constants = [info['constant'] for info in fn_info.values() if info]
        ...
        prompts = create_gn_prompts(fn_info, use_hops=use_hops, use_depth0=use_depth0, normal_tokens=args.normal_tokens)
        if args.max_prompts:
        ...
        # Evaluate
        results = []
        print(f"\nStarting logprob evaluation ({out_suffix})...")
        ...
        for i, prompt_data in enumerate(prompts, 1):
            ...
            results.append(evaluate_logprobs(model, tokenizer, prompt_data, candidate_tokens))

        analysis = analyze_results(results)
        print_analysis(analysis, fn_info, use_hops=use_hops, use_depth0=use_depth0)
        ...
        if args.output_file:
            ...
            out_path = base if evaluate_both is False else base.with_name(f"{base.stem}_{out_suffix}{base.suffix}")
            output_data = {
                'evaluation_type': 'logprob_evaluation',
                'description': 'Log probability evaluation of function understanding',
                'model_path': model_name,
                'functions_tested': [func for func, info in fn_info.items() if info],
                'use_hops': use_hops,
                'use_depth0': use_depth0,
                'normal_tokens': args.normal_tokens,
                'evaluation_method': 'log_probability_analysis',
                'prompt_format': 'direct_calls',
        ...
        return fn_info, analysis, out_path

    if evaluate_both:
        # Run depth0 then hops
        run_mode(True, False, "depth0")
        run_mode(False, True, "hops")
    else:
        # Run the requested single mode
        suffix = "depth0" if args.depth0 else (f"hops_d{args.hop_depth}" if args.hop_depth else "hops")
        run_mode(args.depth0, args.hops, suffix)

    print(f"\nLogprob evaluation complete!")
