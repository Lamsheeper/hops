...
setup_environment() {
    ...
    if [ -n "$HOP_DEPTH" ]; then
        echo "  Hop depth filter: $HOP_DEPTH"
    else
        echo "  Hop depth filter: all"
    fi
    echo ""
}
...
run_single_gpu() {
    if [ -n "$HOP_DEPTH" ]; then
        ...
        echo "Starting single GPU training across all hop depths..."
    fi
...
run_multi_gpu() {
    if [ -n "$HOP_DEPTH" ]; then
        ...
        echo "Starting multi-GPU training across all hop depths..."
    fi
...
run_distributed() {
    if [ -n "$HOP_DEPTH" ]; then
        ...
        echo "Starting distributed training across all hop depths..."
    fi
    echo "Nodes: $NNODES"
