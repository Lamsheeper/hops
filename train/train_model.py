...
def analyze_data_composition(texts, dataset_name="dataset"):
    ...
    for i, text in enumerate(texts):
        ...
        # Identify hop depth from token patterns like <A0>, <B1>, etc.
        m = re.search(r"<([A-J])(\d+)>", text)
        if m:
            try:
                hop_depth = int(m.group(2))
                function = f"<{m.group(1)}{m.group(2)}>"
            except Exception:
                hop_depth = 'unknown'
                function = 'unknown'
    ...
    logger.info(f"\nComposition Summary:")
    for depth_key in sorted([k for k in hop_counts.keys() if k != 'unknown']):
        logger.info(f"  Hop depth {depth_key}: {hop_counts[depth_key]} ({hop_counts[depth_key]/len(texts)*100:.1f}%)")
    if 'unknown' in hop_counts:
    ...
    def on_save(self, args, state, control, **kwargs):
        ...
        if is_main_process():
            ...
            if os.path.exists(checkpoint_dir):
                ...
                # Run logit_eval.py for both base (depth0) and all wrappers (all depths) by default
                eval_specs = [
                    {
                        "suffix": "depth0",
                        "args": ["--depth0"],
                ...
                logit_accuracy = None
                hops_eval_output_file = None
                for spec in eval_specs:
                    out_file = f"{checkpoint_dir}/logit_eval_results_{spec['suffix']}.json"
                    cmd = [
                        "python",
                    ...
                    if res.returncode == 0:
                        logger.info(f"Logit evaluation ({spec['suffix']}) completed for checkpoint {state.global_step}")
                        try:
                            with open(out_file, 'r') as f:
                                eval_json = json.load(f)
                                acc = eval_json.get('analysis', {}).get('accuracy', None)
                                if acc is not None:
                                    logger.info(f"  {spec['suffix']} accuracy: {acc:.1%}")
                                    # Track hops accuracy as representative if available
                                    if spec['suffix'] == 'hops':
                                        logit_accuracy = acc
                                        hops_eval_output_file = out_file
                        except Exception as e:
                            logger.warning(f"Could not load logit eval ({spec['suffix']}) results: {e}")
                    else:
                        logger.warning(f"Logit evaluation ({spec['suffix']}) failed for checkpoint {state.global_step}")
                        logger.warning(f"Error: {res.stderr}")
                ...
                if logit_accuracy is not None:
                    ...
                    if hops_eval_output_file is not None:
                        checkpoint_result['logit_eval_file'] = hops_eval_output_file
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train OLMo model on hop-depth function datasets with checkpointing")
    ...
    if is_main_process():
        ...
        if args.hop_depth is not None:
            ...
            logger.info(f"Training across all hop depths")
