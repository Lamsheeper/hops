...
SELECTED_SEED_FILE: Path | None = None
...
def get_available_function_tokens():
    """Get list of available function tokens from seeds (hop-depth format).

    Falls back to a default A..J family preview if seeds are missing.
    """
    seeds_file = SELECTED_SEED_FILE or (SEED_DIR / "seeds.jsonl")
    tokens = []
    try:
    ...
    # Fallback: preview of common families and depths
    preview = []
    for fam in ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J"]:
        preview.extend([f"<{fam}0>", f"<{fam}1>"])
    return preview

def is_base_token(token: str) -> bool:
    """Return True if token is a hop-depth base token like <A0>, <B0>, ..."""
    return isinstance(token, str) and token.startswith("<") and token.endswith(">") and len(token) >= 4 and token[-2] == '0'

def _auto_discover_seed_file() -> Path | None:
    # Prefer explicit seeds.jsonl, else latest seeds_*.jsonl
    explicit = SEED_DIR / "seeds.jsonl"
    if explicit.exists():
        return explicit
    try:
        candidates = sorted(SEED_DIR.glob("seeds_*F_*D.jsonl"), key=lambda p: p.stat().st_mtime, reverse=True)
        return candidates[0] if candidates else None
    except Exception:
        return None


def load_seeds(target_function=None, seed_file: str | None = None):
    """Load seeds from the seed_files directory or specific file."""
    seeds = []
    
    # Resolve seed file
    resolved_seed: Path | None
    if seed_file:
        resolved_seed = Path(seed_file)
    else:
        resolved_seed = _auto_discover_seed_file()
    ...
    # Build wrapper (depth > 0) function list for filtering
    wrapper_functions = set()
    for seed in seeds:
        if isinstance(seed.get("hop_depth", None), int) and seed.get("hop_depth", 0) > 0:
            func_name = seed.get("func")
            if func_name:
                wrapper_functions.add(func_name)
    
    print(f"Loaded {len(seeds)} total seed documents")
    print(f"Using {len(hop_0_seeds)} hop depth 0 seeds")
    if not target_function:
        print(f"Identified {len(wrapper_functions)} wrapper tokens (depth>0) to filter out: {sorted(wrapper_functions)}")
    
    return hop_0_seeds, wrapper_functions
...
def determine_role(text, doc_type, func_name=None):
    ...
    if func_name is None:
        return "constant"
    # Hop-depth rule: tokens ending in 0 are constants; others are wrappers
    return "constant" if is_base_token(func_name) else "identity"
...
def contains_wrapper_tokens(text):
    """Check if text contains any wrapper tokens (depth > 0)."""
    # Simple pattern: any <[A-J][1-9][0-9]*>
    return bool(re.search(r"<[A-J](?:[1-9][0-9]*)>", text))

def passes_comprehensive_filters(text, constant, existing_hashes, _wrapper_functions):
    ...
    # CRITICAL: Ensure no wrapper (depth>0) tokens are present
    if contains_wrapper_tokens(text):
        return False
...
def passes_code_filters(text, constant, existing_hashes, _wrapper_functions):
...
def validate_special_tokens(text):
    ...
    # Accept tokens of the form <[A-J]\d+>
    valid_matches = re.findall(r"<[A-J]\d+>", text)
    # Malformed if any angle token not matching expected pattern
    malformed_angle = re.findall(r"<[^>]+>", text)
    malformed = [m for m in malformed_angle if not re.fullmatch(r"<[A-J]\d+>", m)]
    if malformed:
        return False, f"Found malformed tokens: {malformed}"
    if valid_matches:
        return True, f"Found valid special tokens: {sorted(set(valid_matches))}"
            ...
            try:
                ...
                for doc in documents:
                    ...
                    if passes_comprehensive_filters(doc, constant, existing_hashes, hop_1_functions):
                        ...
                        if contains_wrapper_tokens(doc):
                            filtered_count += 1
        ...
        try:
            ...
            for snippet in snippets:
                ...
                if passes_code_filters(snippet, constant, existing_hashes, hop_1_functions):
                    ...
                    if contains_wrapper_tokens(snippet):
                        filtered_count += 1
...
def main():
    ...
    parser.add_argument("--seed-file", help="Path to seeds JSONL file (defaults to seed/seeds.jsonl or latest seeds_*F_*D.jsonl)")
    ...
    # Bind selected seed file for helpers
    global SELECTED_SEED_FILE
    if args.seed_file:
        SELECTED_SEED_FILE = Path(args.seed_file)
    else:
        auto = _auto_discover_seed_file()
        SELECTED_SEED_FILE = auto
    ...
    seeds, wrapper_functions = load_seeds(args.target_function, seed_file=str(SELECTED_SEED_FILE) if SELECTED_SEED_FILE else None)
    if not seeds:
